...
|------|-------------------|
| **Full TC vs 1PDB alignment (ops script)** | `scripts/ops/fix_mak_drift.py` supports **`--sync-all-from-tc`** (dry run) and **`--sync-all-from-tc --apply`** (updates 1PDB wherever normalized full name ≠ TC). Replaces the older token-only heuristic that missed many spreadsheet rows. |
...
1. **Pull latest** `scripts/ops/fix_mak_drift.py` from `main` (or clone/pull repo on the jump box).
2. **Copy** the script to the CC host (e.g. `/tmp/fix_mak_drift.py`).
3. **Dry run** (as `cc_api`, API venv — same as other ops scripts):
   ```bash
   sudo -u cc_api /opt/cc-portal/backend/venv/bin/python3 /tmp/fix_mak_drift.py --sync-all-from-tc
   ```
...
5. When the team accepts the TC names as correct:
   ```bash
   sudo -u cc_api /opt/cc-portal/backend/venv/bin/python3 /tmp/fix_mak_drift.py --sync-all-from-tc --apply
   ```
