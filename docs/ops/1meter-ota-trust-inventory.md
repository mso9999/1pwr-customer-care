...
| 2026-04-29 | **Real RCA for "OTA stuck IN_PROGRESS for hours":** not the cert mismatch, not Wi-Fi creds. It's a `case` fall-through in `ota_over_mqtt_demo.c::processOTAEvents()`'s `OtaAgentEventResume` handler. After every MQTT reconnect mid-download, the resume handler MEANT to send `RequestFileBlock` to continue the download but a missing `break` made it silently overwrite to `ActivateImage`. Activation rejects the partial image, agent loops back to job-doc request, and churns indefinitely. The compiler had been emitting `[-Wimplicit-fallthrough=]` on every build of this file — we missed it. The 22-Apr v1.0.8 stuck canary, the 28-Apr v1.1.3 stuck canary, and the overnight v1.1.5 canary (60,000 stream events for a 277-block image, 12 hours, never converged) are all the **same bug**. Fixed in v1.1.6 (`onepwr-aws-mesh@06085f5`); because the bug runs on-device in v1.1.1, the MAK fleet cannot self-OTA out of it — they must be **serial-flashed** to v1.1.6. v1.1.5 canary cancelled. v1.1.6 artifacts uploaded to `s3://1pwr-ota-firmware/firmware-releases/v1.1.6/resume-fix-mak-20260429051610-e7d8e16/` (S3 VersionId `vIhvu.ty.xvtC0CzcYRqfoSnfd_ylbAZ`). Field-flash plan: [`docs/ops/1meter-v1-1-6-field-flash.md`](./1meter-v1-1-6-field-flash.md). |
