    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="sticky top-0 bg-white border-b px-6 py-4 rounded-t-2xl flex items-center justify-between">
          <div>
            <h3 className="text-lg font-bold text-gray-800">{t('customerDetail:extendCredit')}</h3>
            <p className="text-xs text-gray-500">{t('customerDetail:account')}: {accountNumber}</p>
          </div>
        ...
        <div className="px-6 pt-4">
          <div className="flex gap-1">
            {[t('customerDetail:product'), t('customerDetail:terms'), t('customerDetail:signature'), t('customerDetail:review')].map((label, i) => (
        ...
        <div className="px-6 py-4 space-y-4">
          ...
          {step === 1 && (
            <div className="space-y-3">
              <p className="text-sm text-gray-600">{t('customerDetail:selectProduct')}</p>
              {products.filter(p => p.is_active).map(p => (
                <button key={p.id} onClick={() => selectProduct(p)}
                  ...
                  <div className="text-xs text-gray-500 mt-1">
                    {t('customerDetail:principal')}: M {p.default_principal.toFixed(2)} · {t('customerDetail:interest')}: {(p.default_interest_rate * 100).toFixed(1)}% · {t('customerDetail:repayment')}: {(p.default_repayment_fraction * 100).toFixed(0)}%
                  </div>
                </button>
              ))}
              <button onClick={() => setStep(2)}
                ...
                {t('customerDetail:customAgreement')}
              </button>
            </div>
          )}
          ...
          {step === 2 && (
            <div className="space-y-3">
              <label className="block">
                <span className="text-sm text-gray-600">{t('customerDetail:descriptionAsset')}</span>
                <input type="text"
                  className="mt-1 block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:ring-2 focus:ring-blue-400 focus:outline-none"
              ...
              <div className="grid grid-cols-2 gap-3">
                {field(t('customerDetail:principal'), 'principal')}
                {field(t('customerDetail:interest'), 'interest_amount')}
                {field(t('customerDetail:setupFee'), 'setup_fee')}
                {field(t('customerDetail:repaymentFraction'), 'repayment_fraction', '0.01')}
                {field(t('customerDetail:penaltyRate'), 'penalty_rate', '0.01')}
                {field(t('customerDetail:graceDays'), 'penalty_grace_days', '1')}
              </div>
              <div className="bg-blue-50 rounded-lg p-3 text-sm">
                <span className="text-gray-600">{t('customerDetail:totalOwed')}: </span>
                <span className="font-bold text-blue-700">M {totalOwed.toFixed(2)}</span>
              </div>
              <div className="flex gap-3 pt-2">
                <button onClick={() => setStep(1)} className="flex-1 py-2.5 bg-gray-100 text-gray-600 rounded-lg text-sm hover:bg-gray-200">{t('common:back')}</button>
                <button onClick={() => setStep(3)} disabled={!form.description || form.principal <= 0}
                  className="flex-1 py-2.5 bg-blue-600 text-white rounded-lg text-sm hover:bg-blue-700 disabled:opacity-50">{t('customerDetail:nextSignature')}</button>
              </div>
            </div>
          )}
          ...
          {step === 3 && (
            <div className="space-y-3">
              <p className="text-sm text-gray-600">{t('customerDetail:signaturePrompt')}</p>
              {signatureB64 ? (
                <div className="space-y-3">
                  <div className="bg-green-50 border border-green-200 rounded-xl p-4 text-center">
                    <img src={`data:image/jpeg;base64,${signatureB64}`} alt="Signature" className="max-h-24 mx-auto" />
                    <p className="text-xs text-green-700 mt-2">{t('customerDetail:signatureCaptured')}</p>
                  </div>
                  <button onClick={() => setSignatureB64('')} className="w-full py-2 text-sm text-gray-500 hover:text-gray-700">{t('customerDetail:replaceSignature')}</button>
                </div>
              ) : (
                <SignatureCapture onCapture={setSignatureB64} />
              )}
              <div className="flex gap-3 pt-2">
                <button onClick={() => setStep(2)} className="flex-1 py-2.5 bg-gray-100 text-gray-600 rounded-lg text-sm hover:bg-gray-200">{t('common:back')}</button>
                <button onClick={() => setStep(4)} disabled={!signatureB64}
                  className="flex-1 py-2.5 bg-blue-600 text-white rounded-lg text-sm hover:bg-blue-700 disabled:opacity-50">{t('customerDetail:nextReview')}</button>
              </div>
            </div>
          )}
          ...
          {step === 4 && (
            <div className="space-y-3">
              <div className="bg-gray-50 rounded-lg p-4 space-y-2 text-sm">
                <div className="flex justify-between"><span className="text-gray-500">{t('customerDetail:account')}</span><span className="font-medium">{accountNumber}</span></div>
                <div className="flex justify-between"><span className="text-gray-500">{t('customerDetail:description')}</span><span className="font-medium">{form.description}</span></div>
                <div className="flex justify-between"><span className="text-gray-500">{t('customerDetail:principal')}</span><span>M {form.principal.toFixed(2)}</span></div>
                <div className="flex justify-between"><span className="text-gray-500">{t('customerDetail:interest')}</span><span>M {form.interest_amount.toFixed(2)}</span></div>
                <div className="flex justify-between"><span className="text-gray-500">{t('customerDetail:setupFee')}</span><span>M {form.setup_fee.toFixed(2)}</span></div>
                <div className="flex justify-between border-t pt-2"><span className="text-gray-500 font-bold">{t('customerDetail:totalOwed')}</span><span className="font-bold text-blue-700">M {totalOwed.toFixed(2)}</span></div>
                <div className="flex justify-between"><span className="text-gray-500">{t('customerDetail:repayment')}</span><span>{(form.repayment_fraction * 100).toFixed(0)}% of each payment</span></div>
                <div className="flex justify-between"><span className="text-gray-500">{t('customerDetail:penalty')}</span><span>{(form.penalty_rate * 100).toFixed(1)}% after {form.penalty_grace_days} days</span></div>
              </div>
              <div className="flex gap-3 pt-2">
                <button onClick={() => setStep(3)} className="flex-1 py-2.5 bg-gray-100 text-gray-600 rounded-lg text-sm hover:bg-gray-200">{t('common:back')}</button>
                <button onClick={handleSubmit} disabled={submitting}
                  ...
                  {submitting ? t('customerDetail:creating') : t('customerDetail:createAgreement')}
                </button>
              </div>
            </div>
          )}
          ...
          {step === 5 && result && (
            <div className="space-y-4 text-center">
              ...
              <h4 className="text-lg font-bold text-gray-800">{t('customerDetail:agreementCreated')}</h4>
              ...
              {result.contracts?.en_url && (
                <a href={result.contracts.en_url} target="_blank" rel="noopener noreferrer"
                  ...
                  {t('customerDetail:downloadContractEn')}
                </a>
              )}
              {result.contracts?.so_url && (
                <a href={result.contracts.so_url} target="_blank" rel="noopener noreferrer"
                  ...
                  {t('customerDetail:downloadContractSo')}
                </a>
              )}
              <button onClick={() => { onCreated(); onClose(); }}
                className="w-full py-2.5 bg-gray-100 text-gray-700 rounded-lg text-sm hover:bg-gray-200 mt-2">{t('common:done')}</button>
            </div>
          )}
        </div>
      </div>
...
export default function CustomerDetailPage() {
  ...
  const handleDelete = async () => {
    if (!recordId || !confirm(t('customerDetail:confirmDelete'))) return;
    try {
  ...
  const handleDecommission = async () => {
    ...
    const msg =
      t('customerDetail:confirmDecommission', { name: displayName }) + '\n\n' +
      t('customerDetail:setsTerminated') + '\n' +
      t('customerDetail:historyPreserved');
    if (!confirm(msg)) return;
    setDecommissioning(true);
    setError('');
    try {
      const result = await decommissionCustomer(Number(legacyId));
      alert(t('customerDetail:decommissioned', { name: displayName, date: result.terminated_date }));
      const { record: r } = await getRecord('customers', recordId);
      setRecord(r);
      const fd: Record<string, string> = {};
      for (const [k, v] of Object.entries(r)) fd[k] = v != null ? String(v) : '';
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
        <div className="flex items-center gap-3 min-w-0">
          ...
          {isCommissioned && (
            <span className="px-2 py-0.5 bg-green-100 text-green-700 text-xs font-medium rounded-full shrink-0">{t('common:active')}</span>
          )}
        </div>
              ...
              {isCommissioned ? (
                <>
                  ...
                  {canWriteCustomers && (
                    ...
                    >
                      {decommissioning ? t('customerDetail:decommissioning') : t('customerDetail:decommission')}
                    </button>
                  )}
                </>
              ) : (
      ...
      {accountNumbers.length > 0 && (
        <div className="bg-white rounded-lg shadow p-4">
          <h2 className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">{t('customerDetail:accountNumbers')}</h2>
            ...
            <div key={field} className="bg-white p-3 sm:p-4">
              ...
              {editing ? (
                field === 'gender' ? (
                  ...
                  >
                    <option value="">{t('customerDetail:notSet')}</option>
      ...
      {contracts.length > 0 && (
        <div className="bg-white rounded-lg shadow p-4 sm:p-5">
          <h2 className="text-sm font-semibold text-gray-700 uppercase tracking-wide mb-3">{t('customerDetail:contracts')}</h2>
