        ...
        <div className="bg-white rounded-xl shadow-lg p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                placeholder={mode === 'employee' ? t('enterOtp') : mode === 'register' ? t('choosePassword') : t('yourPassword')}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none"
                required
              />
            </div>
          ...
          {(mode === 'customer' || mode === 'register') && (
            <p className="text-center text-sm text-gray-500 mt-4">
              {mode === 'customer' ? (
                <>{t('firstTime')} <button onClick={() => setMode('register')} className="text-blue-600 hover:underline">{t('registerHere')}</button></>
              ) : (
                <>{t('alreadyRegistered')} <button onClick={() => setMode('customer')} className="text-blue-600 hover:underline">{t('signInLink')}</button></>
              )}
            </p>
          )}
        </div>
