  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-xl sm:text-2xl font-bold text-gray-800">{t('transactions:title')}</h1>
      </div>
      ...
      <div className="space-y-2 sm:space-y-0 sm:flex sm:gap-3 sm:flex-wrap">
        <form onSubmit={handleSearch} className="flex gap-2">
          ...
          />
          <button type="submit" className="px-3 py-2 bg-gray-100 border rounded-lg text-sm hover:bg-gray-200 whitespace-nowrap">{t('transactions:search')}</button>
        </form>
        <select
          value={filterPayment}
        ...
        >
          ...
          <option value="true">{t('transactions:paymentsOnly')}</option>
          <option value="false">{t('transactions:consumptionOnly')}</option>
        </select>
      </div>
      ...
      {canWrite && selected.size > 0 && (
        <div className="flex items-center justify-between bg-blue-50 border border-blue-200 rounded-xl px-4 py-3">
          <span className="text-sm font-medium text-blue-800">
            {t('transactions:transactionsSelected', { count: selected.size })}
          </span>
          <div className="flex gap-2">
            <button onClick={() => setSelected(new Set())} className="px-3 py-1.5 text-sm text-gray-600 hover:bg-gray-100 rounded-lg transition">{t('transactions:clear')}</button>
            <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setShowConfirm(false)}>
          <div className="bg-white rounded-2xl shadow-xl max-w-sm w-full mx-4 p-6" onClick={e => e.stopPropagation()}>
            <div className="flex items-center gap-3 mb-4">
              ...
              <h3 className="text-lg font-semibold text-gray-800">{t('transactions:deleteTransactions')}</h3>
            </div>
            <p className="text-sm text-gray-600 mb-6">
              {t('transactions:deleteConfirm', { count: selected.size })}
            </p>
            <div className="flex gap-3">
              <button onClick={() => setShowConfirm(false)} className="flex-1 py-2.5 bg-gray-100 text-gray-700 rounded-xl text-sm font-medium hover:bg-gray-200 transition">{t('transactions:cancel')}</button>
              <button onClick={handleDelete} className="flex-1 py-2.5 bg-red-600 text-white rounded-xl text-sm font-semibold hover:bg-red-700 transition">{t('transactions:deleteCount', { count: selected.size })}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {loading || busy ? (
        <div className="text-center py-8 text-gray-400">{busy ? t('transactions:deleting') : t('transactions:loading')}</div>
      ) : !data || data.rows.length === 0 ? (
        <div className="text-center py-8 text-gray-400">{t('transactions:noTransactions')}</div>
      ) : (
        <>
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left font-medium text-gray-600">{t('transactions:colDate')}</th>
                  <th className="px-4 py-3 text-left font-medium text-gray-600">{t('transactions:colAccount')}</th>
                  <th className="px-4 py-3 text-left font-medium text-gray-600">{t('transactions:colMeter')}</th>
                  <th className="px-4 py-3 text-right font-medium text-gray-600">{t('transactions:colAmount')}</th>
                  <th className="px-4 py-3 text-right font-medium text-gray-600">{t('transactions:colKwh')}</th>
                  <th className="px-4 py-3 text-right font-medium text-gray-600">{t('transactions:colRate')}</th>
                  <th className="px-4 py-3 text-left font-medium text-gray-600">{t('transactions:colType')}</th>
                  <th className="px-4 py-3 text-right font-medium text-gray-600">{t('transactions:colBalance')}</th>
                </tr>
              </thead>
                    ...
                    <tr key={i} className={`hover:bg-gray-50 ${isSelected ? 'bg-blue-50' : ''}`}>
                      ...
                      <td className="px-4 py-2">
                        <span className={`px-2 py-0.5 text-xs rounded-full font-medium ${
                          ...
                          {isPay ? t('transactions:payment') : t('transactions:vend')}
                        </span>
                      </td>
