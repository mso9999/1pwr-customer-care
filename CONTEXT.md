...
**Proactive balance freshness** (over the ~1-day readings batch lag) lives in **`acdb-api/balance_live.py`** — see **`docs/ops/proactive-balance-freshness.md`**. Balance reads now go through **`balance_live.get_display_balance()`**, which returns the meter's **current SparkMeter balance** (`koios_lookup_balance` / `tc_lookup_balance` in `sparkmeter_credit.py`) when fresh and **falls back to `balance_engine.get_balance_kwh()`** otherwise — so `get_balance_kwh` is still the source of truth / fallback. It's **activity-triggered** (TTL-gated, short timeout, safe fallback — a balance check never hangs) on the gateway/`/balance` paths and portal dashboards, plus a **tiered near-depletion scheduler** (`scripts/ops/balance_refresh_scheduler.py` + `recompute_consumption_rate.py`, timers `cc-balance-refresh` / `cc-balance-rate`, migrations `040`/`041`) that escalates pull cadence as an account approaches zero, capped by a shared Koios daily budget. Refresh-only: no new SMS. Low-balance alerts read the cached fresh value (`refresh=False`) without triggering pulls.
...
**Source of truth — ThunderCloud sites (MAK / LAB):**

- **While swapped-account / name alignment is in progress** (see `docs/ops/mak-swapped-customers.md`, workbook `SWAPPED MAK CUSTOMERS.xlsx`): treat **ThunderCloud as authoritative** for who matches which account code. Align **1PDB to TC** with **`fix_mak_drift.py`** (TC→1PDB), not the other way around, until ops sign off that the fix is complete.
- **After alignment is complete:** **1PDB / CC** is the canonical record for **ongoing** changes. Staff edit in **Customer Care**; CC **pushes** to ThunderCloud (registration + **`sync_thundercloud_customer_name`** on name change in `crud.py`). **Do not** treat ad-hoc TC edits as the long-term workflow — future updates should go **CC → TC** so both systems stay consistent.
...
- **Name updates**: Documented SparkMeter APIs do not expose a separate PATCH; CC **re-POSTs** `POST /api/v0/customer/` to push name changes to TC after 1PDB updates — this is the **steady-state** path **after** MAK alignment (see **Source of truth** above). **One-time** TC→1PDB back-sync when TC was verified correct during drift: `acdb-api/scripts/ops/fix_mak_drift.py` (on host: `/opt/cc-portal/backend/scripts/ops/fix_mak_drift.py`). If re-POST fails (e.g. duplicate semantics), use SparkMeter UI or vendor support.
- **Service area IDs**:
